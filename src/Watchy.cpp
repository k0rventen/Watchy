...
void Watchy::showMenu(byte menuIndex, bool partialRefresh) {
  ...
  const char *menuItems[] = {"About Watchy", "Set Sleep","Set Time","Sync Time"};
...
void Watchy::showSleep() {
  ...
  uint16_t w, h;
  display.setFont(&FreeMonoBold9pt7b);
  const char* line1 = "zzz..";
  display.getTextBounds(line1, 0, 150, &x1, &y1, &w, &h);
  display.setTextColor(GxEPD_BLACK);
  display.setFullWindow();
  display.fillScreen(GxEPD_WHITE);
  display.drawBitmap(50, 20, icon_moon, 100, 100, GxEPD_BLACK);
  ...
  RTC.rtc_pcf.clearAlarm(); // resets the alarm flag in the RTC
  RTC.rtc_pcf.setAlarm(settings.alarmMinute-1, settings.alarmHour, 99, 99);
...
bool Watchy::connectWiFi() {
  if (WL_CONNECT_FAILED ==
      WiFi.begin(settings.wifiSsid,settings.wifiPassword)) { // WiFi not setup, you can also use hard coded credentials
...
void Watchy::showSyncTime() {
  ...
  display.print("TZ: ");
  display.println(settings.timezone);
  display.display(false); // full refresh
  if (connectWiFi()) {
    if (getDateAPI()) {
